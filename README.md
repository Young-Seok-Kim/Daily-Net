...
조회 결과는 메뉴 이름 하나당 문서 하나로 저장합니다. **못 찾은 것도 저장합니다** —
없는 걸 매번 다시 물어보는 게 제일 아깝기 때문입니다 (실제로 못 찾는 쪽이 더 많습니다).

문서 이름은 세 토막이고, 각각 **"이 값을 더는 못 믿게 되는 조건"** 입니다.

```
getFoodNtrCpntDbInq02_v2_신라면
└──────┬──────────┘ └┬┘ └─┬─┘
     API 오퍼레이션   규칙판  정규화된 메뉴명
```

> ⚠️ **환산·매칭 규칙을 고치면 `RULES_VERSION`을 올리십시오.**
> 캐시에 담기는 건 API 원본이 아니라 **계산해 낸 결과**입니다. 판을 안 올리면
> 캐시가 먼저 걸려 새 규칙이 아예 실행되지 않습니다. 실제로 겪었습니다 —
> 라떼가 검은콩 라떼로 붙는 걸 고쳐 배포했는데 캐시 때문에 그대로였습니다.
>
> 대상: `MIN_SCORE` · `MAX_RATIO` · `PACK_SPLIT_RATIO` · `FIELD` 매핑 ·
> `normalize` / `pickBest` / `portionFactor` 등 **`lookupFood`의 결과를 바꾸는 것**.
> `quantityOf` · `statedAmountOf`는 캐시 이후 단계라 해당 없습니다.

### TTL — 캐시가 늙지 않도록

문서마다 `expireAt`(기본 90일)을 넣습니다. 두 가지를 한 번에 해결합니다.

* 한 번 캐시되면 영원히 그 값인 문제 → 만료 후 다시 물어봅니다
* `RULES_VERSION`을 올렸을 때 옛 문서가 저절로 사라집니다

**"안 쓰면 지운다"가 아니라 "쓴 지 오래되면 지운다"** 입니다. 문서는 캐시에 없을 때만 쓰므로,
매일 조회되는 메뉴여도 90일에 한 번은 다시 물어봅니다. 읽을 때마다 기한을 늘리면
캐시 적중이 전부 쓰기가 되어 캐시를 쓰는 의미가 없어집니다.

> ⚠️ **필드만 넣는다고 지워지지 않습니다.** Firebase 콘솔에서 정책을 켜야 합니다.
> Firestore → **시간 경과 후 삭제(TTL)** → 정책 만들기 →
> 컬렉션 그룹 `foodCache`, 타임스탬프 필드 `expireAt`
>
> 한 번만 설정하면 그 뒤로는 손댈 일이 없습니다. 켜지 않아도 앱은 정상 동작하며,
> 문서가 지워지지 않을 뿐입니다.
