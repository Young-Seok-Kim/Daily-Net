...
const prompt = `
            ...
            1. **칼로리 산출 로직 고정**:
               ...
               - **양이 명시된 항목은 표준 성분표 값을 그대로 환산하십시오. 재추정하지 마십시오.**
                 ...
                 - **개수로 적힌 양은 그 개수가 사용자가 정한 값입니다.**
                   ...
                   - 여럿이 나눠 먹었다고 함께 적혀 있으면 그 개수는 **전체가 먹은 양**입니다.
                     아래 나눠 먹기 규칙대로 전체를 계산한 뒤 사용자 몫을 나누십시오.
                     ⚠️ **적힌 개수는 전체를 계산할 때 한 번 쓰면 끝입니다.
                     나눈 몫에 그 개수를 다시 곱하면 안 됩니다**
               ...
               - **여럿이 나눠 먹었다고 적었으면 사용자가 먹은 몫만 계산하십시오.**
                 몇 명이 함께 먹었다는 **뜻이면 어떻게 적혔든 해당합니다.**
                 - ⚠️ **해당하는 표현을 일부러 나열하지 않습니다.** 나열하면 그 말이 적혔을 때만
                   나누고, 같은 뜻을 다른 말로 적은 것은 혼자 다 먹은 것으로 계산됩니다.
                   **단어가 아니라 뜻으로 판단하십시오**
                 - 사용자 몫의 중량을 **항목 이름에 g으로 적으십시오.** (예: "○○ 250g")
                 - **나눠 먹은 양이 개수로 적혀 있으면, 먼저 전체(개수 × 한 단위의 양)를
                   계산한 뒤 그것을 사람 수로 나누십시오.** 순서는 곱하고 → 나누기입니다.
                   **나눈 몫이 최종값입니다. 거기서 계산이 끝납니다**
                   - 곱하기와 나누기는 **각각 정확히 한 번**입니다. 실제로 4명이 치킨 2마리를
                     나눠 먹었다는 같은 입력이 "치킨 1마리 800g"(둘 다 안 함),
                     "치킨 0.5마리 ×2 2100kcal"(나눈 몫에 개수를 또 곱함)으로 나왔습니다.
                     맞는 답은 2마리(약 4200kcal) ÷ 4 = **0.5마리 약 1050kcal** 하나뿐입니다
                   - 이름에는 나눈 몫을 그대로 적으십시오. ("치킨 0.5마리 500g"처럼)
                     **×2 같은 배수 표기를 덧붙이지 마십시오.** kcal 칸에도 몫의 열량만 담습니다.
                     사용자가 적은 개수를 몫도 전체도 아닌 다른 개수로 바꿔 적으면,
                     사용자는 자기가 적은 양이 무시됐다고 생각하게 됩니다
                 - 몇 명인지 안 적혀 있으면 나누지 마십시오. 짐작해서 나누면 안 됩니다
                 - **똑같이 나누지 않았다고 적었으면 그 말을 반영하십시오.**
                   남들보다 많이 먹었다는 뜻이면 절반보다 많게,
                   적게 먹었다는 뜻이면 절반보다 적게 잡습니다
                   - 확실하지 않게 적었어도 똑같이 반영하십시오.
                     사용자가 기억하는 대로 적은 것이니 무시하지 마십시오
                   - 얼마나 더 먹었는지 안 적혀 있으면 **6:4 정도로만** 잡으십시오.
                     조금 더 먹었다는 말을 두 배로 해석하면 안 됩니다
