...
const prompt = `
            ...
            4. **종합 평가**: 현재 식단이 체중 감량과 영양 균형에 미치는 영향을 한 줄로 평가하세요.
               - ⚠️ **'evaluation'에는 칼로리 숫자를 적지 마십시오.** 목표치도, 섭취량도 쓰지 마십시오.
                 "2000kcal 대비", "총 1375kcal로" 같은 표현을 모두 금지합니다.
               - 숫자는 리포트의 다른 칸이 이미 정확하게 보여줍니다. 참고로 이 사용자의 목표는
                 하루 ${recommendedCalories} kcal이며, 많고 적음의 판단은 서버가 계산해 따로 표시합니다.
               - 여기에는 **식단 구성에 대한 평가만** 적으십시오.
                 (예: 단백질은 충분하나 채소가 부족하고 지방 비중이 높습니다)
        ...
        // 총평에 칼로리 숫자가 남아 있으면 프롬프트 지시를 어긴 것이다.
        //
        // 실제로 겪었다 — 섭취가 1,375kcal인 날에 "목표 2000kcal 대비 높다"고 적어 보냈다.
        // 바로 위 칸에는 서버가 계산한 "권장 1,738 / 내 섭취 1,375 (권장 이내)"가 찍혀 있어서
        // 한 화면 안에서 두 말이 맞부딪혔다. 모델이 자기가 만든 숫자를 다시 떠올리려다 지어낸 것이다.
        //
        // 텍스트를 고쳐 쓰지는 않는다. 남의 문장을 기계로 손대면 더 이상해진다.
        // 대신 남겨두고 본다. 계속 나오면 프롬프트를 더 조여야 한다는 신호다.
        if (/\d[\d,]*\s*(kcal|칼로리)/i.test(data.evaluation || "")) {
            console.warn(`[eval] 총평에 칼로리 숫자가 들어감: ${data.evaluation}`);
        }
